"""
Test suite for retirement API optional fields with defaults.
Tests that the API handles missing/None values gracefully with proper defaults.
"""
...
def test_real_estate_with_default_values():
    """Test that RealEstateInput handles missing data with defaults."""

    # Test creating a real estate object with no fields provided (should use defaults)
    real_estate = RealEstateInput()
    
    assert real_estate.total_property_value == 0.0
    assert real_estate.total_outstanding_mortgages == 0.0
    assert real_estate.annual_appreciation_pct == 3.0
    assert real_estate.include_in_net_worth is True
    assert real_estate.model_property_sale is False
...
def test_retirement_input_with_missing_real_estate_section():
    """Test that RetirementInput handles missing real_estate section gracefully."""
    ...
    retirement_data = {
        ...
        # Note: No real_estate section provided - this should be handled gracefully
    }

    # This should not raise a validation error
    try:
        input_model = RetirementInput(**retirement_data)
        assert input_model.real_estate is None  # Should be None when not provided
        print("✓ Test passed: RetirementInput handles missing real_estate section")
    except ValidationError as e:
        pytest.fail(f"ValidationError should not occur with missing real_estate: {e}")


def test_retirement_input_with_empty_rental_properties():
    """Test that RetirementInput handles empty rental properties gracefully."""
    ...
    retirement_data = {
        ...
        "income_streams": {
            "rental_properties": []  # Empty list - should be handled gracefully
        }
    }
    ...
    try:
        input_model = RetirementInput(**retirement_data)
        assert input_model.income_streams is not None
        assert len(input_model.income_streams.rental_properties) == 0
        print("✓ Test passed: RetirementInput handles empty rental properties")
    except ValidationError as e:
        pytest.fail(f"ValidationError should not occur with empty rental properties: {e}")


if __name__ == "__main__":
    test_rental_property_with_none_values()
    test_rental_property_with_valid_values()
    test_real_estate_with_default_values()
    test_retirement_input_with_none_rental_properties()
    test_retirement_input_with_missing_real_estate_section()
    test_retirement_input_with_empty_rental_properties()
